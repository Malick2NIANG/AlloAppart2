    ...
    <div>
      ...
      {rooms.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="mt-1 text-sm text-sub">Les messages de vos locataires apparaîtront ici.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {rooms.map((room) => (
            <RoomCard
              key={room.id}
              room={room}
              onClick={() => router.push(`/bailleur/messages/${room.id}`)}
            />
          ))}
        </div>
      )}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2 mb-0.5">
          ...
          {lastMsg && (
            <p className="text-[11px] text-sub shrink-0">{formatTime(lastMsg.createdAt)}</p>
          )}
        </div>
