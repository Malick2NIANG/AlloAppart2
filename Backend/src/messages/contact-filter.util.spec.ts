...
describe('sanitizeContactInfo', () => {
  it('masque un numéro sénégalais écrit avec espaces', () => {
    const { content, wasFiltered } = sanitizeContactInfo(
      'Appelle-moi au 77 123 45 67 stp',
    );
  ...
  it('laisse un prix intact (pas de faux positif)', () => {
    const { content, wasFiltered } = sanitizeContactInfo(
      'Le loyer est de 250 000 FCFA par mois',
    );
  ...
  it('masque une adresse email', () => {
    const { content, wasFiltered } = sanitizeContactInfo(
      'Ecris-moi à mamadou.diop@gmail.com plutôt',
    );
    expect(wasFiltered).toBe(true);
  ...
  it('masque plusieurs adresses email dans le même message', () => {
    const { content, wasFiltered } = sanitizeContactInfo(
      'contact1@yahoo.fr ou contact2@outlook.com',
    );
    expect(wasFiltered).toBe(true);
